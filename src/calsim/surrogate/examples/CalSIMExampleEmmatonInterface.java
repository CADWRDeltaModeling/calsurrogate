...
public class CalSIMExampleEmmatonInterface {
	...
	public float annec_arr(float[] Qsac_prv, float Qsac_est, float[] Qexp_prv, float Qexp_est, float[] Qsjr_prv,
        ...
        ArrayList<double[][]> monthlyInput = new ArrayList<double[][]>(Arrays.asList(sac,exp,dcc,dcd,sjr,tide,smscg));
        
        
        
		ave_type = 0; // TODO hardwired to mean, which isn't right here
		float out = manager.annEC(monthlyInput, location, variable, 
				ave_type, currMonth, currYear);
		return out;
	}
    /**
     * 
     * @param Qsac_prv Array of four past monthly values Sac flow from one month ago to four (reverse chron) 
     ...
     * @param DXC_prv Four Past DXC gate positions, where 1.0 is open and 0.0 is closed
     ...
     * @param Qsac_oth_prv Four past month other northern flows (not ones being varied)
     ...
     * @return
     */
	public float annlinegen_arr(float[] Qsac_prv, float Qsac_est, 
			float[] Qexp_prv, float Qexp_est,
			float[] Qsjr_prv, float Qsjr_fut, float[] DXC_prv,
			float DXC_fut, float[] DICU_prv, float DICU_fut, float[] Qsac_oth_prv, float Qsac_oth_fut,
			float[] Qexp_oth_prv, float Qexp_oth_fut, float[] SMSCG_prv, float SMSCG_fut, float ECTARGET,
			int location, int variable, int ave_type, int currMonth, int currYear) {
...
public static void main(String[] args) {
	...
	int location = 3; int variable=0; int ave_type=0; int currMonth=8; int currYear=1994;
	float out = calsimIF.annec_arr(sac, sac_est, exp, exp_est, 
