...
/**
 * Rational histospline that smoothly distributes monthly totals into daily. The
 * spline is conservative with respect to monthly totals and monthly time
 * boundaries and Given sufficiently high values of the parameters p and q, it
...
public class ConservativeSpline {
	...
	/**
	 * This is the main constructor, and assumes that the full set of input
	 * parameters is provided, including the initial and terminal conditions and
	 * minimum bound
	 * 
	 * @param xval abscissas at which data are provided
	 * @param y    original data. y[k] represents the (average) valued from x[k] to
	 *             x[k+1]
	 * @param pq   array of values for the tension parameters p and q (constrained
	 *             to be the same)
	 ...
	 * @throws Exception
	 */
	public ConservativeSpline(double[] xval, double[] y, double[] pq, double y0, double yn, double ymin)
		...
		n = x.length;
		if (n <= 3)
			throw new IllegalArgumentException("Array arguments to the spline must have more than 3 elements");
	 ...
	 */
	public ConservativeSpline(double[] xval, double[] y, double[] pq, double y0, double yn)
			throws IllegalArgumentException {
	...
	public void fitHistSpline(double[] y, double y0, double yn) throws IllegalArgumentException, Exception {
		double h = 0;
		d = new double[n];
		e = new double[n];
		y1 = new double[n];
	...
	private void RationalSpline1(double[] x, double[] y, double y0, double yn) throws Exception {
		int kp1, km1, k1;
		// double pk21,pk22;
		double pk2, pk, qk2, qk, p22, h, q22, g2, r2, z, p2, q2;
		// double e1=EPS-1.;
		int n1 = n - 1;
		int n2 = n - 2;
		double r1 = 0;
		...
		for (int k = 0; k < n1; k++) {
			...
			if (k != 0) {
				...
				if (k == 1)
					y1[km1] = y1[km1] - qk1 * g1 * y0;
				if (k == (n1 - 1))
					y1[km1] = y1[km1] - pk2 * g2 * yn;
			}
			p21 = p22;
			qk1 = qk2;
			g1 = g2;
	...
	private void tridiu(int nn, double[] aa, double[] bb, double[] cc, double dd[], double eps) throws Exception {
		double h1, h2, h3, z;
		h1 = 0.;
		h2 = 0.;
		h3 = 0.;
		...
		for (int k = 0; k < nn; k++) {
			z = bb[k] - h3 * h1;
			if (Math.abs(z) < eps)
				throw new Exception("Singular problem");
			h1 = cc[k] / z;
			cc[k] = h1;
			h2 = (dd[k] - h3 * h2) / z;
			bb[k] = h2;
	...
	private void intone(double v) throws Exception {
		...
		if (v < x[0] || v > x[n - 1]) {
			...
			throw new Exception("New abscissa value " + v + " out of bounds (extrapolation) compared to original range:"
					+ x[0] + " to " + x[n - 1]);
		}
		if (v >= x[indx]) {
			if (v < x[indx + 1])
	...
	public static void main1(String[] argv) {
		...
		try {
			PrintWriter outfile = new PrintWriter(
					new BufferedWriter(new FileWriter("F:/projects/linegen/linegen/out2.txt")));
			for (int i = 0; i < 276; i++) {
				outfile.write(xnew[i] + "   " + ynew[i] + "\n");
			}
			outfile.close();
