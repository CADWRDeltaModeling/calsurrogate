...
public class TensorWrapper implements Surrogate {
	...
	/**
	 * Create a TensorWrapper given the path to a directory containing saved_bundle.pb
	 * Please keep this code free of complex dependencies and locations that would make it
	 * harder to test.
	 * @param fpath path containing the saved model. 
	 * @param tensorNames names used to access input names for float values inputs
	 * @param tensorNamesInt names used to access input names for integer valued inputs
	 * @param outName name of the output to be querieds
	 */
	public TensorWrapper(String fpath,
			String[] tensorNames,
			String[] tensorNamesInt,
			String outName,
     ...
     * @param rawData Data in the form of doubles. The ArrayList enumerates features, the first array 
     * dimension is batch index and the second is reserved for dimensionality of the input (e.g. time).
     * @param rawDataInt Same as RawData but for long integers. Can be null
     * @return Result of prediction. First dimension is the batch number and second is Tensor dimension of the output (e.g. station)
     */
	@Override
	public float[][] estimate(ArrayList<double[][]>rawData, ArrayList<long[][]> rawDataInt){
		...
		if (nFeaturesInt > 0) {
			...
			if (nBatchInt != nBatch) { System.out.println("Bad integer");}
